...
import argparse, pandas as pd, json, re
...
def load_vocab(path: Optional[str]):
    ...
    for _, r in df.iterrows():
        ...
        names = [canon] + [x.strip() for x in (r.get("brand_names") or "").split("|") if x]
...
def build_vocab(hcpcs_csv: str, ndc_csv: str, regimens_xlsx: str) -> pd.DataFrame:
    ...
    for s in sheets:
        ...
        cand_name_cols = [c for c in df.columns if any(k in c.lower() for k in ["generic", "drug", "name"]) ]
    ...
    def agg_names(df, generic_col, brand_col):
        ...
        g = tmp.groupby(generic_col)[brand_col].apply(lambda x: sorted(set([v for v in x if v and v.lower() != "nan"])) )
        return g
    ...
    def agg_codes(df, generic_col, code_col):
        ...
        g = tmp.groupby(generic_col)[code_col].apply(lambda x: sorted(set([v for v in x if v and v.lower() != "nan"])) )
        return g
    ...
    reg["brand_names"] = reg["canonical_generic"].map(lambda g: sorted(set((brands_hc.get(g, []) or []) + (brands_nd.get(g, []) or []))))
    reg["hcpcs_codes"] = reg["canonical_generic"].map(lambda g: hcpcs_by_generic.get(g, []))
    reg["ndc11_list"] = reg["canonical_generic"].map(lambda g: ndc11_by_generic.get(g, []))
    reg["ndc9_list"] = reg["canonical_generic"].map(lambda g: ndc9_by_generic.get(g, []))
    reg["synonyms"] = reg.apply(lambda r: sorted(set([r["canonical_generic"], *r["brand_names"]])), axis=1)
    reg["rxcui"] = ""

    for c in ["brand_names", "hcpcs_codes", "ndc11_list", "ndc9_list", "synonyms"]:
        reg[c] = reg[c].apply(lambda x: "|".join(x) if isinstance(x, list) else (x or ""))

    reg = reg.dropna(subset=["canonical_generic"]).drop_duplicates(subset=["canonical_generic", "class"])
    return reg


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--hcpcs", default='resources/Hcpcs Medication List-07-14-2025_07_52.csv', required=True)
    ap.add_argument("--ndc", default='resources/Published Ndc Package List-07-14-2025_07_49.csv', required=True)
    ap.add_argument("--regimens", default='resources/ICI_Regimens_PK_06_02_2025.xlsx', required=True)
